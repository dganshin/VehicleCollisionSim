...
public class VehicleRuntimeUI : MonoBehaviour
{
    ...
    private void DrawCollisionSection(SimpleCarController vehicle)
    {
        ...
        changed |= DrawSlider(ref vehicle.inactivePushAssistSpeed, "纵向推车单步上限", 0f, 6f, "m/s");
        changed |= DrawSlider(ref vehicle.inactivePushAssistMaxTargetSpeed, "推车目标速度上限", 0.5f, 10f, "m/s");
        changed |= DrawSlider(ref vehicle.inactivePushAssistImpulseThreshold, "纵向冲量触发阈值", 0f, 800f, "Ns");
        changed |= DrawSlider(ref vehicle.contactImpulseTransferScale, "冲量转速度倍率", 0.2f, 6f, "x");
        changed |= DrawSlider(ref vehicle.contactPushVelocityStep, "静止贴车推送步进", 0f, 1.5f, "m/s");
