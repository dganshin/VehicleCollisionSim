...
public class VehicleRuntimeUI : MonoBehaviour
{
    ...
    private void DrawDriveSection(SimpleCarController vehicle)
    {
        ...
        changed |= DrawSlider(ref vehicle.directionChangeDriveBoost, "换向反向驱动增强", 1f, 3f, "x");
        changed |= DrawSlider(ref vehicle.directionChangeBrakeTorque, "换向刹车扭矩(旧)", 200f, 6000f, "Nm");
        changed |= DrawSlider(ref vehicle.stationaryReverseRpmThreshold, "静止换向轮速阈值", 20f, 600f, "rpm");
        changed |= DrawSlider(ref vehicle.stationaryReverseBrakeTorque, "静止换向刹车扭矩", 0f, 6000f, "Nm");
        changed |= DrawSlider(ref vehicle.stationaryReverseLockTime, "静止换向刹轮时间", 0f, 0.6f, "s");
    ...
    private void DrawCollisionSection(SimpleCarController vehicle)
    {
        ...
        changed |= DrawSlider(ref vehicle.inactivePushAssistMaxTargetSpeed, "推车目标速度上限", 0.5f, 30f, "m/s");
        changed |= DrawSlider(ref vehicle.inactivePushAssistImpulseThreshold, "纵向冲量触发阈值", 0f, 800f, "Ns");
        changed |= DrawSlider(ref vehicle.contactImpulseTransferScale, "冲量转速度倍率", 0.2f, 10f, "x");
        changed |= DrawSlider(ref vehicle.contactPushVelocityStep, "静止贴车推送步进", 0f, 4f, "m/s");
        changed |= DrawSlider(ref vehicle.contactStaticPushSpeedThreshold, "静止贴车判定阈值", 0.2f, 4f, "m/s");
        changed |= DrawSlider(ref vehicle.contactMinimumPushSpeed, "正面顶车最低启动速度", 0f, 4f, "m/s");
        changed |= DrawSlider(ref vehicle.contactImmediateVelocitySeed, "正面顶车即时速度种子", 0f, 5f, "m/s");
        changed |= DrawSlider(ref vehicle.contactPushSpeedThreshold, "低速顶车速度阈值", 0.5f, 40f, "m/s");
