...
本项目开发过程中使用了 AI/Code Agent 作为辅助开发工具。小组成员负责课程需求拆解、功能取舍、Unity 场景运行测试、碰撞与车辆手感验收、问题反馈、代码审阅和最终集成决策；AI/Code Agent 主要用于辅助形成代码修改方案、排查 Git 与 Unity 工程问题、整理复现文档，并根据人工测试反馈辅助迭代车辆控制和碰撞逻辑。

项目没有直接复用完整的第三方成品系统。车辆模型、贴图和城市/高速地图采用第三方免费资源，用于缩短建模和场景搭建时间；课程要求相关的车辆驱动、多车切换、相机跟随、车车碰撞、环境碰撞、可碰撞树木、运行时参数 UI、物理量显示、车轮视觉旋转、材质兼容处理和多轮 bug 修复均在本 Unity 工程中完成。

AI 辅助开发的大致流程为：先由小组按课程要求拆分模块并提出目标效果，再在 Unity 中导入资源、搭建最终场景和运行测试；随后结合测试现象迭代车辆控制、碰撞响应、参数面板和复现流程。每次修改后由小组成员在 Unity 中验证车辆前进/倒车、转向、低速碰撞、推车、撞树、撞环境和多车辆切换效果，再决定是否继续调整。
...
- 车辆与环境对象碰撞：可与城市道路、马路牙子、人行道柱、桥梁、建筑、树木和动态障碍物等对象发生物理碰撞。
- 植物与动态障碍碰撞演示：场景中额外放置 3 棵 `CrashTestTree` 测试树，车辆撞击后可触发倒伏或位移效果；测试树附近会运行时生成 3 个可撞飞圆柱油桶和 5 个交通锥路障，用于展示轻量动态障碍物碰撞。
...
- 文档和测试：整理了 README、环境配置步骤、运行说明和功能验证流程。
...
10. 驾驶车辆撞向测试树附近运行时生成的 `CrashTestTrashCan_01`、`CrashTestTrashCan_02`、`CrashTestTrashCan_03` 和 `CrashTestCone_01` 至 `CrashTestCone_05`，验证轻量动态障碍物可以被车辆撞飞、滚动或冲散。
11. 按 `F2` 打开参数面板，调整驱动力、刹车、阻尼、摩擦、质量等参数，观察车辆运动变化。
12. 观察参数面板中的速度、前向速度、估算加速度、估算纵向合力、质量、重力加速度和估算车长等读数。
...
| 实现车辆与其它对象的物理碰撞 | 已实现车辆与城市道路、马路牙子、人行道柱、桥梁、建筑、3 棵可撞倒测试树、3 个动态圆柱油桶和 5 个交通锥路障的物理碰撞 |
| UI 界面包含运动驱动、力和常用物理参数设置及可视化 | 已实现运行时参数面板和基础物理量可视化 |

当前车辆数量已满足“3-5 辆汽车”的课程要求。
...
│     └─ Physics/
│        └─ CrashableTree.cs
│        └─ RuntimeCollisionPropsSpawner.cs
├─ Packages/
│  └─ manifest.json
├─ ProjectSettings/
│  └─ ProjectVersion.txt
...
| `Assets/Scripts/Physics/RuntimeCollisionPropsSpawner.cs` | 运行时在测试树附近生成可撞飞圆柱油桶和交通锥路障，用于动态障碍物碰撞演示 |
...
车辆模型、贴图和城市地图来自第三方免费资源包。第三方资源仅作为模型、贴图和场景资产使用，不包含本项目的车辆控制系统、碰撞响应逻辑、多车切换逻辑或运行时参数 UI。本项目在这些资源基础上完成车辆控制、碰撞交互、多车管理、UI 参数调节、相机体验、材质兼容、测试树和最终场景集成。

当前验收场景已经保存为 `Assets/Scenes/CollisionTestScene.unity`，打开项目后不需要手动运行 Tools 脚本。该场景覆盖车辆控制、车车碰撞、车辆与树木/动态油桶/交通锥/桥梁/路沿等环境对象碰撞，以及运行时物理参数可视化。
...
- 当前额外配置 3 棵可撞倒测试树、3 个运行时动态油桶和 5 个交通锥路障，用于“车辆与植物对象/动态障碍物碰撞”演示。
